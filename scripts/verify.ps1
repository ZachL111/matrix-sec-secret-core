...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-matrix-sec-secret-detail.ps1

php tests/test_domain_review.php

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-matrix-sec-secret-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-matrix-sec-secret-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-matrix_sec_secret_core-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
